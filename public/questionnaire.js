...
(async function protectQuestionnaire() {
  ...
  try {
    ...
    document.getElementById("selectedPlan").value =
      ({forge:"FORGE Personalizado",essential:"Essential (legacy)",forge_plus:"FORGE+ (legacy)",coaching:"Coaching (legacy)"})[data.plan] || data.plan;
...
form.addEventListener("submit", async e => {
  ...
  try {
    ...
    localStorage.setItem("forgePortalCode", result.portal_code || access.code);
    form.hidden = true;
