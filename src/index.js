const PLANS = {
  forge: { name: "FORGE Personalizado", amount: 500 },

  // Legacy plans kept only so old test orders/profiles continue to display correctly.
  essential: { name: "Essential (legacy)", amount: 299 },
  forge_plus: { name: "FORGE+ (legacy)", amount: 499 },
  coaching: { name: "Coaching (legacy)", amount: 799 }
};
...
const SELLABLE_PLANS = new Set(["forge"]);
...
async function createBankOrder(request, env) {
  ...
  if (!plan || !SELLABLE_PLANS.has(planKey)) {
...
Genera un BORRADOR para revisión de Alex. Debe ser práctico, claro y fácil de mostrar después al cliente.

FORMATO OBLIGATORIO:

FORGE — RUTINA PERSONALIZADA
ENFOQUE: una frase breve

DÍA 1 — Nombre del día
Ejercicio | Series | Repeticiones | RIR/RPE | Descanso
Ejercicio | Series | Repeticiones | RIR/RPE | Descanso

DÍA 2 — Nombre del día
Ejercicio | Series | Repeticiones | RIR/RPE | Descanso

[continúa según los días reales del cliente]
...
SUSTITUCIONES
- Ejercicio original → alternativa

NOTAS DE ALEX
- puntos que Alex debe revisar antes de entregar

REGLAS:
- La sesión debe caber en el tiempo disponible.
- Cada fila debe contener UN SOLO ejercicio principal; nunca combines "Ejercicio A o Ejercicio B" en la misma fila.
- Las alternativas deben ir únicamente en SUSTITUCIONES.
- Mantén una selección biomecánicamente coherente con el objetivo del día.
- No uses LaTeX ni símbolos técnicos innecesarios.
- No uses frases promocionales como "de élite".
- No indiques llegar al fallo salvo que exista una razón clara y apropiada para el nivel.
- Adapta volumen, frecuencia y selección de ejercicios al nivel y objetivo.
- No inventes diagnósticos médicos.
- Si una limitación requiere valoración clínica, indícalo dentro de NOTAS DE ALEX y programa de forma conservadora.
`.trim();
}
  ...
  try {
    const answer = await callForgeAI(env, {
      ...
      instructions:
        ...
        "Responde en español, directo y accionable. Por defecto usa menos de 120 palabras. " +
        "Usa el perfil del cliente y considera oficial únicamente la rutina entregada por Alex. " +
        "Si piden una sustitución, prioriza el mismo patrón de movimiento y el equipo disponible. " +
        "Para una sustitución responde con este formato: SUSTITUCIÓN RECOMENDADA; Ejercicio; Series; Reps; RIR/RPE; Descanso; Alternativa 2; Por qué. " +
        "No uses LaTeX, no muestres símbolos como $\\rightarrow$, no uses frases exageradas como de élite. " +
        "No indiques entrenar al fallo salvo que la rutina oficial lo contemple y sea apropiado. " +
        "Puedes explicar descansos, progresión y adaptar una sesión a menos tiempo. " +
        "No cambies de forma importante la programación oficial sin indicar que requiere revisión de Alex. " +
        "No diagnostiques lesiones ni enfermedades. Ante dolor intenso, lesión aguda o síntomas preocupantes, " +
        "indica detener el ejercicio relevante y buscar valoración profesional. No prometas resultados garantizados.",
      input: context,
      maxOutputTokens: 900
    });
