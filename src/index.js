...
const ROUTINE_MODEL_DEFAULT = "gpt-5.6-terra";
const CHAT_MODEL_DEFAULT = "gpt-5.6-luna";
...
async function callOpenAI(env, { model, instructions, input, maxOutputTokens = 1800 }) {
  if (!env.OPENAI_API_KEY) {
    const error = new Error("FORGE AI todavía no está configurado.");
  ...
  if (!response.ok) {
    console.error("OpenAI error", response.status, JSON.stringify(data).slice(0, 1200));
    const error = new Error(data?.error?.message || "No se pudo generar la respuesta de IA.");
  ...
  if (!text) {
    const error = new Error("La IA respondió sin contenido.");
    error.code = "AI_EMPTY";
    throw error;
  }
...
function aiError(error) {
  if (error?.code === "AI_NOT_CONFIGURED") {
    return json({
      error: "FORGE AI todavía no está configurado. Agrega OPENAI_API_KEY como Secret en Cloudflare."
    }, 503);
  }

  console.error(error);
  return json({
    error: "FORGE AI no pudo responder en este momento. Intenta nuevamente."
  }, 502);
}
...
async function adminGenerateRoutine(request, env) {
  ...
  const model = env.OPENAI_ROUTINE_MODEL || ROUTINE_MODEL_DEFAULT;
...
`.trim();
  ...
  const model = env.OPENAI_CHAT_MODEL || CHAT_MODEL_DEFAULT;
