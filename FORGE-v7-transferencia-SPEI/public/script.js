...
async function checkPaymentStatus(orderId, plan) {
  try {
    ...
    if (data.status !== "approved" && data.status !== "rejected") {
      clearTimeout(paymentPollTimer);
      paymentPollTimer = setTimeout(() => checkPaymentStatus(orderId, plan), 5000);
    }
  } catch (err) {
    console.error(err);
    clearTimeout(paymentPollTimer);
    paymentPollTimer = setTimeout(() => checkPaymentStatus(orderId, plan), 8000);
  }
}
...
markTransferSent.addEventListener("click", async () => {
  ...
  try {
    ...
    checkPaymentStatus(activeTransferOrder.order_id, activeTransferOrder.plan);
  } catch (err) {
    console.error(err);
...
// Restore a pending or approved purchase when the client returns.
(function restorePaymentState() {
  ...
  try {
    ...
    checkPaymentStatus(pending.orderId, pending.plan);
  } catch (_) {}
})();
